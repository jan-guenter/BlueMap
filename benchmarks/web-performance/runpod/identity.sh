...
[[ -r /sys/fs/cgroup/cgroup.controllers ]] || {
    printf 'ERROR: cgroup v2 is required\n' >&2
    exit 1
}
[[ -r /sys/fs/cgroup/cpu.max ]] || {
    printf 'ERROR: cgroup v2 cpu.max is unavailable\n' >&2
    exit 1
}
[[ -r /sys/fs/cgroup/cpuset.cpus.effective ]] || {
    printf 'ERROR: cgroup v2 cpuset.cpus.effective is unavailable\n' >&2
    exit 1
}

cpu_max="$(< /sys/fs/cgroup/cpu.max)"
read -r cpu_quota cpu_period cpu_max_extra <<<"$cpu_max"
[[ -z "${cpu_max_extra:-}" && "$cpu_period" =~ ^[1-9][0-9]*$ ]] || {
...
if [[ "$cpu_quota" != "max" ]]; then
    [[ "$cpu_quota" =~ ^[1-9][0-9]*$ ]] || {
        printf 'ERROR: cgroup v2 CPU quota is malformed\n' >&2
        exit 1
    }
    ((cpu_quota % cpu_period == 0)) || {
        printf 'ERROR: cgroup v2 CPU quota is not an integral vCPU allocation\n' >&2
        exit 1
    }
