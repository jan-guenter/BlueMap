...
class ControllerSourceTests(unittest.TestCase):
    ...
    def test_entrypoint_verifies_init_and_main_image_identity(self) -> None:
        ...
        required = (
            ...
            "running controller image identities did not settle",
            '"$current_pod_uid" == "$controller_pod_uid"',
            "controller Pod UID changed during image identity verification",
            "duplicate named container statuses",
            '"$init_image_id" =~ @sha256:[0-9a-f]{64}$',
        ...
        self.assertLess(
            ...
            entrypoint.index("for ((image_attempt = 1;"),
            "immutable Pod manifest references must fail before status retries",
        )

    def test_entrypoint_rejects_ambiguous_status_image_identity(self) -> None:
        entrypoint = read_text(ENTRYPOINT)
        start_marker = """status_identity="$(
        jq -ce '
"""
        end_marker = """
        ' <<<"$pod_json"
...
"""
        self.assertEqual(
            entrypoint.count(start_marker),
            1,
            "status identity jq filter start must be unique",
        )
        self.assertEqual(
            entrypoint.count(end_marker),
            1,
            "status identity jq filter end must be unique",
        )
        jq_filter = entrypoint.split(start_marker, 1)[1].split(end_marker, 1)[0]
        digest = "sha256:" + "a" * 64

        def status(
            *,
            init_ids: list[str | None],
            controller_ids: list[str | None],
            uid: str = "controller-pod-uid",
        ) -> dict[str, Any]:
            def entries(name: str, image_ids: list[str | None]) -> list[dict[str, Any]]:
                return [
                    {"name": name, "imageID": image_id}
            ...
            return {
                "metadata": {"uid": uid},
                "status": {
                    "initContainerStatuses": entries(
                        "prepare-runpod-credentials", init_ids
                    ),
                    "containerStatuses": entries("controller", controller_ids),
                },
            }

        def parse(document: dict[str, Any]) -> subprocess.CompletedProcess[str]:
        ...
        expected = f"ghcr.io/jan-guenter/controller@{digest}"
        settled = parse(
            status(init_ids=[expected], controller_ids=[expected])
        )
        self.assertEqual(settled.returncode, 0, settled.stderr)
        self.assertEqual(
            json.loads(settled.stdout),
            {
                "podUid": "controller-pod-uid",
                "initImageId": expected,
                "controllerImageId": expected,
            },
        )

        missing = parse(status(init_ids=[expected], controller_ids=[]))
        self.assertEqual(missing.returncode, 0, missing.stderr)
        self.assertIsNone(json.loads(missing.stdout)["controllerImageId"])

        for document in (
            status(init_ids=[expected, expected], controller_ids=[expected]),
            status(init_ids=[expected], controller_ids=[expected], uid=""),
            {
                "metadata": {"uid": "controller-pod-uid"},
                "status": {
                    "initContainerStatuses": [
                        {
                            "name": "prepare-runpod-credentials",
                            "imageID": 123,
                        }
                    ],
                    "containerStatuses": [
                        {"name": "controller", "imageID": expected}
                    ],
                },
            },
        ):
            with self.subTest(document=document):
                self.assertNotEqual(parse(document).returncode, 0)
...
class FormalControllerManifestTests(unittest.TestCase):
    ...
    def test_runpod_identity_is_a_stable_single_file_mount(self) -> None:
        spec = pod_spec(self.job)
        identity_volumes = [
            volume for volume in spec.get("volumes", []) if "configMap" in volume
        ]
        self.assertEqual(
            identity_volumes,
            [
                {
                    "name": "runpod-identity",
                    "configMap": {
                        "name": "bluemap-perf-runpod-identity",
                        "defaultMode": 0o444,
                        "items": [{"key": "identity.json", "path": "identity.json"}],
                    },
                }
            ],
            "the RunPod identity must come from one exact ConfigMap key",
        )
        controller = spec.get("containers", [])
        self.assertEqual(len(controller), 1, "Job should have one controller container")
        identity_mount = mount_by_path(
            controller[0], "/runpod-identity/identity.json"
        )
        self.assertEqual(
            identity_mount,
            {
                "name": "runpod-identity",
                "mountPath": "/runpod-identity/identity.json",
                "subPath": "identity.json",
                "readOnly": True,
            },
            "a sole ConfigMap subPath bind-mount provides a stable non-symlink file",
        )
        self.assertEqual(
            [
                mount
                for mount in controller[0].get("volumeMounts", [])
                if mount.get("name") == "runpod-identity"
            ],
            [identity_mount],
            "the controller must not also receive the symlinked ConfigMap directory",
        )
