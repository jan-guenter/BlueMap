...
class TrackedFreezerTests(unittest.TestCase):
    ...
    def test_execute_freeze_locks_the_canonical_output_root(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            manifest_path = root / "manifest.json"
            write_json(manifest_path, {"mapIds": ["world"]})
            output_dir = root / "formal-run"
            controller_hashes = {
            ...
            args = Namespace(
                ...
                load_generator_image=self.LOAD_GENERATOR_IMAGE,
            ...
            with (
                patch.object(
                    ...
                    return_value=(controller_hashes, "a" * 64),
                ),
                patch.object(orchestrate, "validate_repository_freeze"),
                patch.object(
                    orchestrate,
                    "validate_run_root",
                    return_value=output_dir.resolve(),
                ),
                patch.object(orchestrate, "Kubectl"),
                patch.object(
                    orchestrate,
                    "acquire_global_lock",
                ...
                args.confirm = freeze.required_confirmation(
                    self.LOAD_GENERATOR_IMAGE
                )
                with self.assertRaisesRegex(RuntimeError, "stop after lock"):
                    freeze.execute_freeze({}, {}, args)
            acquire_lock.assert_called_once_with(output_dir.resolve())
