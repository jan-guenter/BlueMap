...
representation from being returned.
...
readiness checks. The shutdown grace is a total budget shared by HTTP draining
and database-pool cleanup.

`max_in_flight_requests` is a non-queuing map-response limit (default `32`).
...
return to the pool. Set this below the default when large textures and the pod
memory budget require it.
